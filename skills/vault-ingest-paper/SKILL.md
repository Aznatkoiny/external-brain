...
Read `system/protocol.md`, `system/schema.md`, `system/relationships.md`, `templates/source-record.md`, and `skills/obsidian-markdown/SKILL.md`. Use the environment's PDF skill when available.

## Prepare the source

1. Ensure the original is under `sources/papers/`. Do not alter it after ingestion.
2. Run `python3 scripts/extract_pdf.py "sources/papers/Paper.pdf"`. Reuse the hash-addressed extraction if reported.
...
2. Create one source record from `templates/source-record.md`; store the PDF path and exact SHA-256.
...
7. Put possible contradictions in `system/review-queue.md` as `potential-conflict`; do not resolve them autonomously.
8. Update maps, `system/index.md`, and `system/log.md`, then run `python3 scripts/vault_lint.py`.
