...
Read `vault/system/protocol.md`, `vault/system/schema.md`, `templates/thought.md`, and `skills/obsidian-markdown/SKILL.md`. Read relevant existing pages before editing them.

## Workflow

1. Search the index, aliases, filenames, and content for concepts or questions implicated by the thought.
2. Create `vault/sources/thoughts/YYYY-MM-DD - Short descriptive title.md`. Preserve the user's material verbatim; clearly label any cleanup such as removed filler or corrected transcription.
...
6. Explain each important wikilink in a sentence. Add uncertain semantic judgments to `vault/system/review-queue.md`.
7. Update `vault/system/index.md` and prepend a `capture` entry to `vault/system/log.md`.
